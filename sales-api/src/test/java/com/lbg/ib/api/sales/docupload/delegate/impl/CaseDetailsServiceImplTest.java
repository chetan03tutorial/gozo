...
import com.lbg.ib.api.sales.docupload.service.impl.CaseDetailsServiceImpl;
...
public class CaseDetailsServiceImplTest {
    ...
    @Mock
    CaseServiceValidator validator;
    ...
    @Mock
    CaseDelegate caseDelegateService;
    ...
    public void shouldReturnExpectedResult() {
        ...
        PowerMockito.when(caseServiceHelper.mapToCaseDTO(Mockito.any(Case.class))).thenReturn(caseDTO);
        // PowerMockito.
        Mockito.when(httpRequest.getAttribute(DocUploadConstant.BRAND_DISPLAY_VALUE)).thenReturn(brandValue);
        /*
        ...
        Mockito.when(serviceHelper.getProcessDto()).thenReturn(processDTO);
        
        Mockito.when(validator.validationForCreateCase(Mockito.any(CaseDTO.class))).thenReturn(true);
        
        Mockito.when(caseDelegateService.createCase(Mockito.any(CreateCaseRequestDTO.class))).thenReturn(caseDTO);
        
        Mockito.when(httpRequest.getSession()).thenReturn(session);
        
        ServiceResponse<Case> response = caseServiceImpl.createCase(requestBody);
        Assert.assertNotNull(response);
        /*
         * Assert.assertEquals(caseDTO.getBrand(),
         * response.getResult().getBrand());
    ...
    public void testShouldThrowDocUploadServiceException() {
        ...
        Mockito.when(serviceHelper.getProcessDto()).thenReturn(processDTO);
        Mockito.when(processDTO.getDefaultExpiryPeriod()).thenReturn(100);
        /*
        ...
        Mockito.when(validator.validationForCreateCase(Mockito.any(CaseDTO.class)))
    ...
    @Test
    public void testShouldThrowDocUploadServiceExceptionWithExistingExistingCaseid() {
        ...
        CaseResponseDTO caseResponseDTO = new CaseResponseDTO();
        caseResponseDTO.setServiceResponse("Success");
        List<CaseDTO> listCaseDTO = new ArrayList<CaseDTO>();
        listCaseDTO.add(caseDTO);
        caseResponseDTO.setCaseDetails(listCaseDTO);
        when(searchCaseService.searchCaseDeleteService(any(String.class),any(String.class))).thenReturn(withResult(caseResponseDTO));
    ...
    public void ShouldThrowDocUploadServiceExceptionWrongCreater() {
        ...
        Mockito.when(errorCodeMapper.resolve(ResponseErrorConstants.INVALID_CREATOR_ID)).thenReturn(responseErrorVO);
        // Mockito.when(validator.validateCaseDtoWithPartyDtls(Mockito.any(CaseDTO.class))).thenThrow(
        // new DocUploadServiceException(responseErrorVO));
    ...
    public void testUploadDocument() {
        ...
        Mockito.when(bodyMappingHelper.mapRequestBody(Mockito.anyString(), Mockito.eq(CaseDTO.class)))
                .thenReturn(casedto);
        Mockito.when(bodyMappingHelper.mapRequestBody(requestBody, Case.class)).thenReturn(caseDetails);
        Mockito.when(caseServiceHelper.mapToCaseDTO(Mockito.any(Case.class))).thenReturn(casedto);
        Mockito.when(uploadDocumentValidator.validateUploadDetails(Mockito.any(CaseDTO.class), Mockito.anyInt(),
                Mockito.any(CaseDTO.class))).thenReturn(true);
        Mockito.when(
                uploadDocumentValidator.validateFileUploadCount(Mockito.any(CaseDTO.class), Mockito.any(CaseDTO.class)))
                .thenReturn(true);
        Mockito.when(serviceHelper.getProcessCode()).thenReturn("proces_cde");
        Mockito.when(uploadDocumentDelegate.uploadDocument(Mockito.any(CaseDTO.class), Mockito.any(InputStream.class)))
                .thenReturn("docId");
        CaseDTO updateCaseResponse = new CaseDTO();
        updateCaseResponse.setCaseReferenceNo("ABC");
        Mockito.when(updateCaseDelegate.updateCaseDetails(Mockito.any(CaseDTO.class), Mockito.anyString()))
                .thenReturn(updateCaseResponse);
        /*
         * Mockito.when( getCaseDelegate.retrieveDocUploadCaseData(Mockito.any(
         * SearchCaseRequestDTO.class))) .thenReturn(casedto);
         */
        Mockito.when(httpRequest.getAttribute(DocUploadConstant.JWT_SID_HEADER)).thenReturn("asfsd");
        Mockito.when(httpRequest.getSession()).thenReturn(session);
        Mockito.when(session.getAttribute("createdCase")).thenReturn(casedto);
        /*
        ...
        ServiceResponse<Case> response = caseServiceImpl.uploadDocument(requestBodyJson, new MultipartBody(atts));
    ...
    public void testUploadDocumentWithInvalidAttach() {
        ...
        Mockito.when(caseServiceImpl.uploadDocument(requestBody, new MultipartBody(atts)))
                .thenThrow(DocUploadServiceException.class);
        System.out.println("here 1");
        System.out
                .println("caseServiceImpl" + caseServiceImpl + "\nrequestBodyJson" + requestBodyJson + "\natts" + atts);
        // caseServiceImpl.uploadDocument(requestBodyJson, new
        // MultipartBody(atts));
    }
    ...
    public void testUploadDocumentWithInvalidAttachIO() throws IOException {
        ...
        Mockito.when(errorCodeMapper.resolve(ResponseErrorConstants.INVALID_REQUEST_STRUCTURE))
                .thenReturn(new ResponseErrorVO());
        Mockito.when(caseServiceImpl.uploadDocument(requestBody, new MultipartBody(atts))).thenThrow(IOException.class);
        System.out.println("here 1");
        System.out
                .println("caseServiceImpl" + caseServiceImpl + "\nrequestBodyJson" + requestBodyJson + "\natts" + atts);
    ...
    public void testUploadDocumentWithInvalidAttachConfig() throws IOException {
        ...
        Mockito.when(errorCodeMapper.resolve(ResponseErrorConstants.INVALID_REQUEST_STRUCTURE))
                .thenReturn(new ResponseErrorVO());
        Mockito.when(caseServiceImpl.uploadDocument(requestBody, new MultipartBody(atts)))
                .thenThrow(ConfigurationServiceException.class);
        System.out.println("here 1");
        System.out
                .println("caseServiceImpl" + caseServiceImpl + "\nrequestBodyJson" + requestBodyJson + "\natts" + atts);
        caseServiceImpl.uploadDocument(requestBodyJson, new MultipartBody(atts));
     ...
     * testconvertFilesToByesIOException(){ Attachment att=null;; try { att =
     * new Attachment("json", new ByteArrayResource(new
     * byte[1]).getInputStream(), new
     * ContentDisposition(DocUploadConstant.CONTENT_DISPOSITION_VALUE)); } catch
     * (IOException e) { // TODO Auto-generated catch block e.printStackTrace();
     * } Mockito.when(caseServiceImpl.convertFilesToByes(att,
     * null)).thenThrow(IOException.class);
     * Mockito.when(errorCodeMapper.resolve(ResponseErrorConstants.
     * INVALID_REQUEST_STRUCTURE)) .thenReturn(new ResponseErrorVO());
     * 
    ...
    public void testPreviewDocument() {
        ...
        Mockito.when(bodyMappingHelper.mapRequestBody(requestBody, CaseDTO.class)).thenReturn(caseDTO);
        Mockito.when(documentValidator.validateDocumentService(Mockito.any(CaseDTO.class), Mockito.any(Boolean.class)))
                .thenReturn(true);
        Mockito.when(serviceHelper.getProcessCode()).thenReturn("abc");
        Mockito.when(httpRequest.getAttribute(DocUploadConstant.JWT_SID_HEADER)).thenReturn("asfsd");
        Mockito.when(previewDocumentDelegate.retrieveDocumentation(Mockito.any(CaseDTO.class))).thenReturn(file);
    ...
    public void testPreviewDocumentErrorCase() {
        ...
        Mockito.when(previewDocumentDelegate.retrieveDocumentation(Mockito.any(CaseDTO.class)))
                .thenThrow(new DocUploadServiceException(responseErrorVO));
        Response response = caseServiceImpl.previewDocument(requestBody);
    ...
    public void testPreviewDocumentForNullResponse() {
        ...
        Mockito.when(serviceHelper.getProcessCode()).thenReturn("abc");
        Mockito.when(httpRequest.getAttribute(DocUploadConstant.JWT_SID_HEADER)).thenReturn("asfsd");
        Mockito.when(errorCodeMapper.resolve(ResponseErrorConstants.FAILED_TO_RETRIEVE_FILE))
