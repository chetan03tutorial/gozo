...
import com.lbg.ib.api.sales.dao.product.holding.domain.AccGroupType;
...
@Component
public class PcaEligibilityRequestMapper {
    ...
    public DetermineEligibleCustomerInstructionsRequest mapRequest(
            PcaEligibilityRequestDTO determineEligibleCustomerInstructionsRequestDTO) {
     ...
     */
    public DetermineEligibleCustomerInstructionsRequest populateRequest(PcaEligibilityRequestDTO requestDTO) {
    ...
    private DetermineEligibleCustomerInstructionsRequest populateHeadersAndProductArrangements(
            PcaEligibilityRequestDTO requestDTO) {
        ...
        if (requestDTO.getAccounts() != null) {
            rq.setExistingProductArrangments(populateExistingProductArrangements(requestDTO));
        }

        /*
         * if (requestDTO.existingProductArrangements() != null) {
         * rq.setExistingProductArrangments((ProductArrangement[])
         * (requestDTO.existingProductArrangements())); }
         */
        return rq;
    }
     ...
     */
    private Customer populateCustomerDetails(PcaEligibilityRequestDTO requestDTO) {
        ...
        if (null != primaryInvParty.getTinDetails() && null != primaryInvParty.getTinDetails().getTaxResidencies()) {
    ...
    private List<com.lbg.ib.api.sales.soapapis.commonapi.businessobjects.PostalAddress> setAddressInformation(
        ...
        if (null != primaryInvParty.getCurrentAddress()) {
     ...
     */
    private Individual populateIsPlayedBy(PcaEligibilityRequestDTO requestDTO, PrimaryInvolvedParty primaryInvParty) {
        Individual isPlayedBy = new Individual();
        isPlayedBy.setIndividualName(new IndividualName[] { setIndividualName(primaryInvParty) });
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
        Date dob;
        try {
            dob = formatter.parse(primaryInvParty.getDob());
            isPlayedBy.setBirthDate(calendar(dob));
        ...
        if (null != primaryInvParty.getTinDetails()) {
            nationalities = primaryInvParty.getTinDetails().getNationalities();
            isPlayedBy.setNationality(nationalities.toString()
                    .substring(1, nationalities.toString().lastIndexOf(CLOSING_TAG)).toString());
    ...
    private ProductArrangement[] populateExistingProductArrangements(PcaEligibilityRequestDTO requestDTO) {

        List<Account> accounts = requestDTO.getAccounts();
        String brand = requestDTO.getBrand();
        brand = reverseBrandName(brand);

        List<ProductArrangement> listProductArrangement = new ArrayList<ProductArrangement>();
        for (Account account : accounts) {

            String legalEntity = account.getSellingLegalEntity();
            if (!legalEntity.equals(brand)) {
                continue;
            }
            ProductArrangement productArrangement = new ProductArrangement();
            productArrangement.setAccountNumber(account.getAccountNumber());
            productArrangement.setLifecycleStatus(account.getLifecyclestatus());
            productArrangement.setRelatedEvents(populateEvents(account.getEvents()));
            Organisation organisation = new Organisation();
            OrganisationUnit[] organisationUnits = new OrganisationUnit[1];
            OrganisationUnit organisationUnit = new OrganisationUnit();
            organisationUnit.setSortCode(account.getSortCode());
            organisationUnits[0] = organisationUnit;
            organisation.setHasOrganisationUnits(organisationUnits);
            productArrangement.setFinancialInstitution(organisation);
            Product product = new Product();
            ExtSysProdIdentifier[] prodIdentifiers = new ExtSysProdIdentifier[1];

            ExtSysProdIdentifier prodIdentifier = new ExtSysProdIdentifier();
            prodIdentifier.setSystemCode("00004"); // Needs to remove this hard
            prodIdentifier.setProductIdentifier(account.getProductIdentifier()); // coding
                                                                                 // once
                                                                                 // PAM
                                                                                 // db
            // service starts sending
            // this field
            prodIdentifiers[0] = prodIdentifier;
            product.setProductIdentifier(account.getProductIdentifier());
            product.setProductType(AccGroupType.AccGroupValue.getProductTypeValue(account.getProductType()));
            product.setExternalSystemProductIdentifier(prodIdentifiers);
            productArrangement.setAssociatedProduct(product);
            listProductArrangement.add(productArrangement);
        }

        ProductArrangement[] productArrangements = listProductArrangement
                .toArray(new ProductArrangement[listProductArrangement.size()]);

        return productArrangements;
    }

    public String reverseBrandName(String brand) {
        Map<String, Object> map = configurationDAO.getConfigurationItems("REVERSE_PRODUCT_BRAND");
        Object value = map.get(brand);
        if (value != null) {
            return (String) value;
        }
        return null;
    ...
    private RequestHeader prepareSoapHeaders() {
        ...
        logger.traceLog(this.getClass(), "PCA DECI Branch Context is : " + session.getBranchContext());
        ...
        if (null != session.getUserContext()) {
            Map<String, Object> map = configurationDAO.getConfigurationItems("ChannelIdMapping");
    ...
    private String[] populateEvents(List<Event> events) {
        String[] relatedEvents = new String[events.size()];
        int i = 0;
        for (Event event : events) {
            relatedEvents[i++] = event.getEventId();
        }
