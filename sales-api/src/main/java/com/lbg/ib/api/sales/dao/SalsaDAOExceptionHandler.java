 ...
 */
@Component
public class SalsaDAOExceptionHandler {

    public static final Map<Class, ExceptionHandler> EXCEPTION_HANDLERS = createMapOfExceptionHandlers();

    private static Map<Class, ExceptionHandler> createMapOfExceptionHandlers() {
        Map<Class, ExceptionHandler> exceptionHandlers = new HashMap<Class, ExceptionHandler>();
    ...
    private interface ExceptionHandler<E extends Exception> {
        DAOError handleException(E exception, LoggerDAO logger, Class<?> className, String methodName,
                Object requestDTO);
